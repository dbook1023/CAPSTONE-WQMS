...
function renderReportsList(reports, page = 1) {
    ...
    const rowsHTML = paginatedData.map(r => {
        ...
        const statusClass = (r.overall_status || 'PASS').toLowerCase();
        const reportCode = r.report_code || formatReportId(r.id, r.created_at);
        const fountainName = r.fountain_name || 'Compliance Report';
        const generatorName = r.generated_by_name || 'Admin';

        return `
        <div class="report-row">
            <div class="report-main-group">
                <div class="report-doc-icon">
                ...
                <div class="report-details-box">
                    <div class="report-title-line">
                        ...
                        <span class="report-status-badge ${statusClass}">${r.overall_status}</span>
                    </div>
                    <div class="report-meta-line">
                        <span>By ${generatorName}</span>
                        <span class="meta-dot">•</span>
                        <span>${r.readings_count} Samples</span>
                        <span class="meta-dot">•</span>
                        <span>${createdAt}</span>
                    </div>
                </div>
            ...
            <div class="report-action-buttons">
                <button onclick="viewReport(${r.id})" class="action-icon-btn" title="View Report">
                ...
                <button onclick="triggerReportArchive(${r.id})" class="action-icon-btn danger" title="Archive Report">
    ...
    <div class="table-footer">
        <div class="table-info">
            Showing <span>0</span> to <span>0</span> of <span>0</span> reports
        </div>
        <div class="pagination"></div>
    </div>`;
    
    recentSection.innerHTML = headerHTML + rowsHTML + footerHTML;
